...
历史章节是方法或当时记录，不代表当前执行授权。

### 2026-09-17 阶段研究结论

- 已建立：公共 trace 小样分析、同一 Django task 的两个真实 attempt，以及登记范围内的工具生命周期、容器和宿主资源观测。G1 通过仅适用于集中裁定范围。
- 已观察：[P2-01 对比](reports/analysis/P2-01-review-v1/summary.md) 中，RUN-02 Agent 容器的 CPU、wall、内核内存峰值均高于 RUN-01；它们不是受控重复实验，不能据此解释差异原因或模型效率。
- 尚未证明：完整等待时间、独占工具 CPU、真实容器函数热点、跨任务代表性及 CPU 架构需求。CPU-01 的合成进程热点不能替代 Agent 热点；正式 I/O 仍为 null。
- 必交研究缺口：真实热点解释、样本覆盖与适用范围仍属于 G2；若提出架构或扩展性结论，需受控证据。暂停 CPU-02 不取消这些要求。优化 A/B 为可选项，自研特权监督器不是研究交付目标。
- 推进方式：按既有 P3-01 条款先审计现有 trace 的状态、依赖和输出验证条件，与 P2 交错设计；不跳过 G3-R，不启动 executor、并发或 scale，不宣称项目已完成。

### Evidence → Allowed Claim（2026-09-20）

| 证据 | 允许结论 | 不允许结论 |
| --- | --- | --- |
| 一个 task 的两个真实 attempt、四个容器 scope | 登记环境可测性、这两次运行的描述性差异 | 两个独立 workload、类别代表性、因果性能优劣 |
| 合成 worker 的 perf/机制试验 | 该目标及条件下的采样/生命周期机制证据 | 真实 Agent 热点、跨平台可用性 |
| 同 task 多次重复 | 本任务、固定条件下的运行间变异 | 将重复当作独立 task 扩大样本量 |
| 预注册筛选的多个独立 task | 在已报告覆盖范围内的类别描述性模式 | 未覆盖类别或总体上的普遍规律 |
| 控制任务/模型/环境的资源或跨平台对照 | 对被控制变量的条件性敏感性结论 | 从混杂运行推断 cache/SIMD/架构需求 |
| 权限失败及完整日志 | 当时身份/平台/操作下的 unavailable | PMU 永远不可用、采样成功、G2 通过 |
| P3-01 调用恢复、关联与配置推导 | 现有 trace 的可恢复信息和缺口 | 正确性或资源保真 Replay |

正式试点按 task × repeat 组织，报告不同 task 数、计划/实际 attempt 数、成功/失败/缺失数、逐点数据及同 task median/IQR；明确分位数算法（提案为线性插值）。失败不删除、不当作零成本，成功条件下的资源统计与全部 attempt 的失败率分开。类别汇总先按 task 聚合，避免某个 task 重复更多而获得更大权重。小样本不主打 P95/P99；bootstrap 如有充分独立 task，应按 task 聚类/分层重采样，不将所有 run 当成独立样本。3 task × 3 repeat 是可审批的试点提案，不是统计充分性标准。

下一批的复现登记要求见 [复现与版本契约](docs/reproducibility.md)。没有观测到的历史平台状态保持 unknown，不以当前快照倒填；锁定包版本与保留运行 identity 是不同层次的工作。
